...
public class TimeSheetCreator {
  ...
  public HSSFWorkbook createTimeSheet(Map<YearMonth, DaysOfMonth> daysByYearMonth, Locale locale)
      throws IOException {
    ...
    for (Map.Entry<YearMonth, DaysOfMonth> entry : daysByYearMonth.entrySet()) {
      ...
      rowCount =
          createRowsForEachDayInMonth(
              daysOfMonth,
              sheet,
              rowCount,
              borderStyle,
              cellStyleForTrueValue,
              cellStyleForFalseValue);
  ...
  private int createRowsForEachDayInMonth(
      DaysOfMonth daysOfMonth,
      HSSFSheet sheet,
      int rowCount,
      CellStyle cellStyle,
      CellStyle styleForTrueValue,
      CellStyle styleForFalseValue) {
  ...
  public String booleanToString(boolean b, Locale locale) {
    if (locale.equals(Locale.forLanguageTag("pl-PL"))) {
      return b ? "TAK" : "NIE";
    } else return b ? "YES" : "NO";
  }

  private void checkCellIfConditionAndPutStyle(
      CellStyle styleForTrueValue, CellStyle styleForFalseValue, Cell cell) {
    if (cell.getBooleanCellValue()) {
      cell.setCellStyle(styleForTrueValue);
    } else cell.setCellStyle(styleForFalseValue);
  }
  ...
  private void workingTimeCountingCell(
      DaysOfMonth daysOfMonth, HSSFSheet sheet, int rowCount, CellStyle cellStyle) {
    HSSFRow row = sheet.createRow(rowCount);
    row.createCell(2)
        .setCellValue(
            daysOfMonth.countWorkingDays(daysOfMonth, daysOfMonth.days().get(1).getWorkingDays()));
    setRowsCellStyle(row, cellStyle);
  }
